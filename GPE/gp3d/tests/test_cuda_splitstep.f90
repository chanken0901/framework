program test_cuda_splitstep
  ...
  real(dp), parameter :: field_tolerance = 5.0e-10_dp
  real(dp), parameter :: scalar_tolerance = 5.0e-10_dp
  ...
  real(dp) :: field_error, cpu_norm, gpu_norm, cpu_energy, gpu_energy
  integer :: i, j, k

  call gp3d_grid_init(grid, 4, 4, 4, 2.0_dp * pi, 2.0_dp * pi, 2.0_dp * pi)
    ...
    do j = 1, grid%ny
      do i = 1, grid%nx
        cpu_state%psi(i,j,k) = cmplx( &
          ...
          0.05_dp * sin(grid%y(j) - grid%z(k)), kind=dp)
        cpu_state%potential(i,j,k) = 0.02_dp * ( &
          cos(grid%x(i)) + sin(grid%y(j)) + cos(grid%z(k)))
      end do
    end do
  end do
  ...
  call gp3d_normalize(cpu_state, grid, params%norm)
  gpu_state = cpu_state

  call gp3d_fft_init(fft_plan, grid%nx, grid%ny, grid%nz)
  call gp3d_gpu_init(gpu, grid, need_argle=.false., device=0)
  call gp3d_gpu_upload(gpu, gpu_state, grid)

  call gp3d_step_split_operator(cpu_state, grid, params, fft_plan)
  call gp3d_gpu_step(gpu, grid, params)
  call gp3d_gpu_download(gpu, gpu_state)
  field_error = maxval(abs(cpu_state%psi - gpu_state%psi))
  if (field_error > field_tolerance) then
    write(*,'(a,es16.8)') "real-time field error: ", field_error
    error stop "CUDA real-time split-step differs from CPU reference"
  end if

  cpu_norm = gp3d_density_norm(cpu_state, grid)
  cpu_energy = gp3d_energy(cpu_state, grid, params, fft_plan)
  call gp3d_gpu_diagnostics(gpu, grid, params, gpu_norm, gpu_energy)
  if (abs(cpu_norm - gpu_norm) > scalar_tolerance) then
    error stop "CUDA norm differs from CPU reference"
  end if
  if (abs(cpu_energy - gpu_energy) > scalar_tolerance) then
    write(*,'(a,es16.8)') "energy error: ", abs(cpu_energy - gpu_energy)
    error stop "CUDA energy differs from CPU reference"
  end if
  ...
  if (field_error > field_tolerance) then
    ...
    error stop "CUDA imaginary-time split-step differs from CPU reference"
  end if

  call gp3d_gpu_finalize(gpu)
  call gp3d_fft_finalize(fft_plan)
  write(*,'(a,es16.8)') "CUDA split-step comparison passed; max error = ", field_error
end program test_cuda_splitstep
