...
導出できる。過去データにghost cellが含まれる場合は中央領域を切り出す。

Usage:
  python tools/slf_to_paraview_merged_cropghost.py output -o paraview --meta output/meta.json --derive auto
  python tools/slf_to_paraview_merged_cropghost.py output -o paraview --meta output/meta.json --derive gpe
  python tools/slf_to_paraview_merged_cropghost.py output -o paraview --derive gpe --fields density,phase --stride 2
  python tools/slf_to_paraview_merged_cropghost.py output -o paraview --meta output/meta.json --derive nse
"""
...
def merge_step(
    ...
    """一つのステップに属する全rankを読み、重なりなく全体3次元場へ結合する。"""
    records = []
    for path in files:
        slf = read_slf(path)
    ...
    is_single_global = len(records) == 1 and records[0][3] is None and first.shape[:3] == (gx, gy, gz)
    if is_single_global:
        ...
        for path, slf, step_read, rank in records:
            if step_read != step:
