...
SIMULATION_INPUT_LAYOUT = [
    ("equation", "physics.model"),
    ("case_name", "case_id"),
    ("initial_condition", "flow.type"),
    ...
    ("use_mpi", "solver.use_mpi"),
    ("use_openmp", "solver.use_openmp"),
]

NSE_INPUT_LAYOUT = [
    ("nse", [
        ("nv", "physics.nse.nv"),
        ("nghost", "physics.nse.nghost"),
        ("gamma", "physics.nse.gamma"),
        ("small_rho", "physics.nse.small_rho"),
        ("small_p", "physics.nse.small_p"),
        ("rho0", "physics.nse.rho0"),
...
def generate_input_dat(data: Dict[str, Any], source: Path) -> str:
    ...
    for output_key, yaml_path in SIMULATION_INPUT_LAYOUT:
    ...
    if physics_model in {"nse", "navier_stokes", "compressible_nse", "compressible_euler"}:
        ...
        lines.append("")
        lines.append("&gpe")
        for _, items in GPE_INPUT_LAYOUT:
            for output_key, yaml_path in items:
                value = get(data, yaml_path, "")
                if value != "":
                    lines.append(f"  {output_key} = {namelist_value(value)}")
...
- Reference density: {get(data, "physics.nse.rho0")}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate input.dat, README.md, case_meta.json from case.yaml with duplicate check."
    )
...
def main() -> int:
    ...
    try:
        ...
        if args.only in {"all", "meta"}:
            write(case_dir / "case_meta.json", generate_meta_json(data, yaml_path), args.overwrite, args.dry_run)
