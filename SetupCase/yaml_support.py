#!/usr/bin/env python3
"""PyYAMLがない環境でもケース設計書を読める安全なYAMLローダー。

PyYAMLが利用可能なら ``safe_load`` を使用し、なければ本フレームワークで
使用するmapping/list/scalarだけを扱う簡易パーサーへ切り替える。
任意オブジェクトの生成やタグ実行は許可しない。
"""
...
def _without_comment(raw: str) -> str:
    """引用符内の#を残し、それ以外のYAMLコメントだけを除去する。"""
    single = False
    double = False
    escaped = False
    for index, char in enumerate(raw):
        ...
        if char == '"' and not single:
            double = not double
            continue
        if char == "#" and not single and not double and (index == 0 or raw[index - 1].isspace()):
            return raw[:index]
    return raw
...
def _split_mapping(text: str, line: int) -> tuple[str, str]:
    ...
    raise YamlFormatError(f"line {line}: expected a key:value mapping")
...
def _split_inline_items(text: str, line: int) -> list[str]:
    items: list[str] = []
    start = 0
    depth = 0
    quote = ""
    ...
    for index, char in enumerate(text):
        ...
        if char in {"'", '"'}:
            ...
            depth -= 1
            if depth < 0:
                raise YamlFormatError(f"line {line}: invalid inline collection")
        elif char == "," and depth == 0:
...
def _scalar(text: str, line: int) -> Any:
    ...
    if text.startswith("[") and text.endswith("]"):
        body = text[1:-1].strip()
        return [] if not body else [_scalar(item, line) for item in _split_inline_items(body, line)]
    if text.startswith("{") and text.endswith("}"):
        body = text[1:-1].strip()
        result: dict[str, Any] = {}
        if not body:
            return result
        for item in _split_inline_items(body, line):
            key, value = _split_mapping(item, line)
            result[key.strip("'\"")] = _scalar(value, line)
        return result
...
class _SubsetParser:
    """インデントに基づいて限定的なYAML mapping/listを再帰的に読む。"""
    ...
    def parse(self) -> Any:
        ...
        if self.tokens[0].indent != 0:
            raise YamlFormatError(
                f"line {self.tokens[0].line}: top-level content must not be indented"
            )
        value = self._block(0)
    ...
    def _sequence(self, indent: int) -> list[Any]:
        ...
        while self.index < len(self.tokens):
            ...
            if item:
                if ":" in item:
                    raise YamlFormatError(
                        f"line {token.line}: mappings inside sequences require PyYAML"
                    )
...
def load_yaml(path: str | Path) -> Any:
    """UTF-8 YAMLを安全に読み、PyYAMLまたは内蔵パーサーの結果を返す。"""
    """Load YAML with PyYAML when available, otherwise use the documented subset."""
    source = Path(path)
...
def dump_yaml(value: Any) -> str:
    """Serialize framework data as YAML, or JSON when PyYAML is unavailable."""
    try:
        import yaml  # type: ignore
    except ImportError:
