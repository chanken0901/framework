...
通常の計算は[Fortran版手順](REACTORS.md)を使用する。
本書は`reference/python/`へ移したオフライン比較用実装の記録。
...
FrameWorkルートで実行する。比較専用`run_reference_reactor.py`を入口とする。
...
python SolverLibrary/ReactingFlow/tools/run_reference_reactor.py "$mechanism" --temperature 1000 --pressure 101325 --mass-fractions '{"H2":0.0285,"O2":0.2264,"N2":0.7451}' --end-time 0.001 --mode constant_volume --output reactor_reference_cv.json
...
python3 SolverLibrary/ReactingFlow/tools/run_reference_reactor.py "$mechanism" --temperature 1000 --pressure 101325 --mass-fractions '{"H2":0.0285,"O2":0.2264,"N2":0.7451}' --end-time 0.001 --mode constant_volume --output reactor_reference_cv.json
