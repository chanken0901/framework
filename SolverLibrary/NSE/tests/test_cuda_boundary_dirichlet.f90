program test_cuda_boundary_dirichlet
  ...
  if (maximum_error > 1.0e-13_dp) then
    write(*,'(A,ES24.16)') 'CUDA Dirichlet mismatch = ', maximum_error
    error stop 'CUDA and CPU Dirichlet boundaries differ'
  end if
  write(*,'(A)') 'CUDA Dirichlet boundary comparison passed'
...
end program test_cuda_boundary_dirichlet
