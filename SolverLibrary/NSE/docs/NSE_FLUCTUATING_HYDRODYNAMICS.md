...
- 対応：直交一様格子、全周期境界、CPU MPI/OpenMP、CUDA／MPI+CUDA、固定／CFL可変時間刻み。
- 未対応：非周期境界、一般座標、多成分・反応流への揺らぎ追加。
- 有効時は粘性・熱伝導を専用の前進勾配／随伴発散演算子に置換する。
  **通常の六次精度粘性演算子へ単に乱数を加える仕様ではない**。
  交差微分を含む輸送全体は空間一次精度（軸方向単独の拡散は二次精度）。
- 対流はKEEP2、KEEP6、WENO5Z＋Roe、HYBRIDを選択可能。
  WENO/HYBRIDの人工的な数値散逸に対応する熱雑音は追加しない。
  したがって物理的輸送と雑音の整合性とは別に、数値散逸による揺らぎの減衰があり得る。
  特に平衡統計を調べる場合はKEEP6を推奨し、格子・時間刻み依存性を確認する。
...
dtの安定性を各ステップで確認する。負密度／負圧等は元状態へ戻して異常終了する。
受理・棄却による統計の偏りを避けるため、乱数の再抽選や雑音のクリップは行わない。
`time.use_fixed_dt: false`では、ステップ開始状態から移流・拡散制約でdtを決定し、
MPI全rankで最小値を共有した後、そのdtで新しい雑音を生成する。
雑音振幅は毎回のdtで評価する。更新失敗後のdt半減・再試行は可変刻みでも禁止する。
Brownian bridgeを用いた棄却型の確率適応積分は未実装。
可変刻みで時間平均を評価する場合は、単純なステップ平均ではなく経過時間で重み付けする。
可変刻みの長時間平衡統計の精度は別途収束検証が必要。
...
安定性・異常判定のスカラー通信と出力用転送も残る。dtがいずれかのrankで
...
全周期境界は現実装の制約。KEEP6・固定dtはデモ既定値であり必須条件ではない。
...
case.yamlにはCENTRAL6、SSPRK3、正のReとPr、全周期境界を指定する。
対流は`numerics.convective_scheme`で`keep2`、`keep6`、`weno5z_roe`、`hybrid`から選択する。
固定刻みでは`time.use_fixed_dt: true`と正の`time.dt`を指定する。
可変刻みでは`time.use_fixed_dt: false`と`numerics.cfl`を指定する。
